...
public class ModelRendererTurbo {
    ...
    public void addCylinder(float x, float y, float z, float radius, float length, int segments, float baseScale, float topScale, int baseDirection, int textureCircleDiameterW, int textureCircleDiameterH, int textureH) {
        ...
        for (int index = 0; index < segments; index++) {
            ...
            float f0 = (pi / segments) * index * 2F + (!dirTop ? 0 : pi);
            float uSize = (float) Math.sin(f0) * (0.5F * uCircle - 2F * uOffset);
            float vSize = (float) Math.cos(f0) * (0.5F * vCircle - 2F * vOffset);
            float f1 = (pi / segments) * index2 * 2F + (!dirTop ? 0 : pi);
            float uSize1 = (float) Math.sin(f1) * (0.5F * uCircle - 2F * uOffset);
            float vSize1 = (float) Math.cos(f1) * (0.5F * vCircle - 2F * vOffset);
    ...
    public void doMirror(boolean x, boolean y, boolean z) {
        for (int i = 0; i < faces.length; i++) {
            ...
            for (int j = 0; j < verts.length; j++) {
                verts[j] = new PositionTextureVertex(verts[j].x * (x ? -1 : 1), verts[j].y * (y ? -1 : 1), verts[j].z * (z ? -1 : 1), verts[j].texturePositionX, verts[j].texturePositionY, verts[j].texturePositionW);
            }
            if (x ^ y ^ z)
                faces[i].flipFace();
        }
    ...
    public void copyTo(PositionTextureVertex[] verts, TexturedPolygon[] poly, boolean copyGroup) {
        ...
        for (int i = 0; i < verts.length; i++) {
            vertices[vertices.length - verts.length + i] = verts[i];
        }
        faces = Arrays.copyOf(faces, faces.length + poly.length);
        for (int i = 0; i < poly.length; i++) {
            faces[faces.length - poly.length + i] = poly[i];
        }
    }
    ...
    private List<Polygon> compilePoly;

    public List<Polygon> compile() {
        if (compilePoly != null) return compilePoly;
        if (field_1402_i || !showModel) {
            compilePoly = Collections.emptyList();
            return compilePoly;
        }

        compilePoly = new ArrayList<>();

        for (TexturedPolygon face : faces) {
            Polygon poly = face.makePoly();
            poly.translate(rotationPointX, rotationPointY, rotationPointZ);
            poly.rotate(rotateAngleX + field_78795_f, rotateAngleY + field_78796_g, rotateAngleZ + field_78808_h);
            if (poly.Vertex.length != 0)
                compilePoly.add(poly);
        }
        return compilePoly;
    }
